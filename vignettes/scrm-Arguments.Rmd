...
* `-t <$\theta$>`: Set the mutation rate to `$\theta = 4N_0u$`, we _u_ is the neutral mutation rate per locus. If this options is given, scrm generates the segregating sites output.
* `-T`: Output the local genealogies in newick format.
* `-O`: Output the local genealogies in the `oriented forest` format as described in [Kelleher _et al._ (2014)](http://dx.doi.org/10.1016/j.tpb.2014.05.001).
* `-L`: Output the TMRCA and the local tree length for each segment (behaves different to ms).
* `-oSFS`: Outputs the the site frequency spectrum. Requires that the mutation rate $\theta$ is given with the '-t' option.
